...
Public Class Main
    ...
    Private Sub CmdConnect_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CmdConnect.Click
        ...
        'Controllo l'esistenza su filesystem del certificato nel caso di Open e Cisco:
        Dim PathVPN As String = Rstdb("Tipoconnessione").Value
        If PathVPN = "Cisco VPN" OrElse PathVPN = "Open VPN" Then
            If PathVPN = "Cisco VPN" Then
                PathVPN = My.Computer.FileSystem.SpecialDirectories.ProgramFiles & "\Cisco Systems\VPN Client\profiles\"
                If File.Exists(PathVPN & Rstdb("connessionestringa").Value.ToString & ".pcf") = False Then
                    MsgBox("Certificato " & Rstdb("connessionestringa").Value.ToString & ".pcf non trovato nei profili della VPN Cisco" & vbCrLf & _
            ...
            Else
                ...
                'Dim OpenDir As New DirectoryInfo(PathVPN.ToString)
                'Dim DirList As New ArrayList(OpenDir.GetDirectories)
                'TOLGO il ciclo for padre perchè mi serve solo se devo ciclare nelle subdirectory:
                'For a = 0 To DirList.Count - 1
                'Dim FileList As New ArrayList(Directory.GetFiles(PathVPN & DirList(a).Name))
...
NoOpenVPNDir:
                If OvpnFound = False Then
                    MsgBox("Certificato " & Rstdb("connessionestringa").Value.ToString & " non trovato nei profili della OpenVPN" & vbCrLf & _
                           "Aggiornare le VPN dal menù Strumenti per collegarsi automaticamente", MsgBoxStyle.Information)
                    Rstdb.Close()
                    Exit Sub
                End If
        ...
        'E' Cisco:        
        If Rstdb("Tipoconnessione").Value = "Cisco VPN" Then
            'VPN CISCO:
            ...
            Dim p As New ProcessStartInfo
            'Dim Cetiri As String = My.Computer.Info.OSVersion
            'PER ORA NO, PERCHE' MI VIENE IL POPUP DELLA DISCONNESSIONE NELLA VPNGUI-->DISCONNETTO LA VPN NEL CASO IN CUI FOSSE GIA' COLLEGATA:
            ...
            'SE UTENZA PERSONALE CHIEDO USER E PASSWORD: ---LO TOLGO PERCHE' 
            'LE METTO TUTTE LE VON CON USER E PWD E FACCIO SCEGLIERE!
            'If Rstdb("utenteconnessione").Value.ToString = "personale" Then
            '    p.Arguments = "-c " & """" & Rstdb("connessionestringa").Value.ToString & """"
            'Else
            'ALTRIMENTI LANCIO LA VPN CON USER E PASSWORD:
            p.Arguments = "-c -sd" & " -user " & Rstdb("utenteconnessione").Value.ToString & " -pwd " & Rstdb("passwordconnessione").Value.ToString & " " & _
        ...
        ElseIf Rstdb("Tipoconnessione").Value = "Open VPN" Then
            ...
            If TxtUser.Text <> "" Then TxtUser.Visible = True
            If TxtPwd.Text <> "" Then TxtPwd.Visible = True
        ...
        ElseIf Rstdb("Tipoconnessione").Value = "Microsoft VPN" Then
            ...
            Using LeggiMVpn As New StreamReader(Application.StartupPath & "\vpn_hdmanager\Mvpn\rasphone.pbk")
                ...
                If MVpn.Contains(Rstdb("nomeconnessione").Value.ToString) = False Then
                    MsgBox("VPN Microsoft " & Rstdb("nomeconnessione").Value.ToString & " non trovata nella rubrica" & vbCrLf & _
                           "Aggiornare le VPN dal menù Strumenti per collegarsi automaticamente", MsgBoxStyle.Information)
                    Rstdb.Close()
                    Exit Sub
                End If
            ...
            Using BatMVpn As New StreamWriter(Application.StartupPath & "\vpn_hdmanager\Mvpn\MVpn.bat")
                BatMVpn.Write("@echo off" & vbCrLf & _
                              "rasdial /disconnect" & vbCrLf & _
                              "rasdial " & """" & Rstdb("nomeconnessione").Value.ToString & """" & " " & _
                              Rstdb("utenteconnessione").Value.ToString & " " & Rstdb("passwordconnessione").Value.ToString & _
                              " /phonebook:" & """" & Application.StartupPath & """" & "\vpn_hdmanager\Mvpn\rasphone.pbk" & vbCrLf & _
                              "@echo on")
    ...
    Private Sub HowToToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles HowToToolStripMenuItem.Click
        HowTo.Show()
    End Sub
    Private Sub ElencoMenu_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ElencoMenu.Click, TurnistiMenu.Click, GruppoCMenu.Click, GruppoBMenu.Click, GruppoAMenu.Click
        If My.Computer.Name.ToUpper.StartsWith("VMHD") Then
